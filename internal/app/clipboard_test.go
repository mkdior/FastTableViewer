...
func stubClipboard(t *testing.T, tools map[string]bool, env map[string]string, goos string, wsl bool) *[]string {
	...
	clipRun = func(name string, args []string, text string) error {
...
func TestClipboardCommandPreference(t *testing.T) {
	all := map[string]bool{"wl-copy": true, "xclip": true, "xsel": true, "pbcopy": true, "clip.exe": true}
	cases := []struct {
		name string
		...
		{"wsl prefers clip.exe", nil, "linux", true, "clip.exe"},
		{"macOS prefers pbcopy", nil, "darwin", false, "pbcopy"},
		{"wayland prefers wl-copy", map[string]string{"WAYLAND_DISPLAY": "wayland-0"}, "linux", false, "wl-copy"},
		{"x11 prefers xclip", map[string]string{"DISPLAY": ":0"}, "linux", false, "xclip"},
		{"no hints take the first installed", nil, "linux", false, "wl-copy"},
	}
	for _, tc := range cases {
		stubClipboard(t, all, tc.env, tc.goos, tc.wsl)
		name, _, ok := clipboardCommand()
		if !ok || name != tc.want {
			t.Errorf("%s: got %q ok=%v, want %q", tc.name, name, ok, tc.want)
		}
	}
